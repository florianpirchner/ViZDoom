...
def createNetwork():
    ...
    W_fc2 = weight_variable([512, actions_num])
    b_fc2 = bias_variable([actions_num])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    # printing
    a_file = open("logs_" + "Doom" + "/readout.txt", 'w')
    h_file = open("logs_" + "Doom" + "/hidden.txt", 'w')

    # get the first state by doing nothing and preprocess the image to 80x80x4
    do_nothing = np.zeros(actions_num)
    do_nothing[0] = 1
    x_t, r_0, terminal = game_state.screen_buffer
    ...
    while "flappy bird" != "angry bird":
        ...
        if t % FRAME_PER_ACTION == 0:
            if random.random() <= epsilon:
                ...
                a_t[random.randrange(actions_num)] = 1
            else:
